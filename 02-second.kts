...
data class Pair<T>(var left: T, var right: T)
...
input.split("\n")
        ...
        .map { findEvenDivisions(it) }
        .filter { it.right != 0  }
        .forEach {
            result += it.left / it.right
        }

print(result)

fun findEvenDivisions(list: List<Int>): Pair<Int> {
    var i = 0
    while (i < list.size) {
        var j: Int = -1 // so that we can put the increment ahead of continue
        while (j < list.size - 1) {
            j++
            if (i == j)
                continue
